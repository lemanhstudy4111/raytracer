	...
	if (isect != null) {
		if ((isect.material.kr != null || isect.material.kt != null) && depth < 5) {
			if (isect.material.kr != null) {
				let reflRay = new Ray(
					isect.position.clone(),
					reflect(ray.d.clone().negate(), isect.normal.clone())
				);
				color.add(
					isect.material.kr.clone().multiply(raytracing(reflRay, depth + 1))
				);
			}
			if (isect.material.kt != null) {
				let refrRay = new Ray(
					isect.position.clone(),
					refract(ray.d.clone(), isect.normal.clone(), isect.material.ior)
				);
				color.add(
					isect.material.kt.clone().multiply(raytracing(refrRay, depth + 1))
				);
			}
		}
		let ambientL = ambientLight.clone();
		if (isect.material.ka != null) {
			let shapeKa = isect.material.ka.clone();
			color.add(ambientL.multiply(shapeKa));
		} //add ambient light
		let shadings = shading(ray, isect);
		color.add(shadings); //add diffuse and specular
	}
...
function shading(ray, isect) {
	let color = new THREE.Color(0, 0, 0);
	// ===YOUR CODE STARTS HERE===
	//Per pseudocode shading in class
	for (let i = 0; i < lights.length; i++) {
		let ls = lights[i].getLight(isect.position);
		// if (ls == null) return color;
		let shadowRay = new Ray(isect.position.clone(), ls.direction.clone());
		let distToLight = ls.position.clone().sub(isect.position.clone()).length();
		let shadowIsect = rayIntersectScene(shadowRay);
		if (shadowIsect != null && shadowIsect.t < distToLight) {
			continue; //region is shadowed
		} else {
			let l = ls.direction.clone(); //light vector l
			let n = isect.normal.clone(); //surface normal n
			let v = ray.d.clone().negate(); //view direction of shading point v
			let r = reflect(l, n); //reflective light vector r
			let ldotnMax = Math.max(l.dot(n), 0); //max(l.n, 0)
			let rdotvMax = Math.pow(Math.max(r.dot(v), 0), isect.material.p); //max(r.v, 0)^p
			let idotkd = new THREE.Color(0, 0, 0);
			if (isect.material.kd != null)
				idotkd.add(
					ls.intensity
						.clone()
						.multiply(isect.material.kd.clone())
						.multiplyScalar(ldotnMax)
				); //diffuse formula
			let idotks = new THREE.Color(0, 0, 0);
			if (isect.material.ks != null)
				idotks.add(
					ls.intensity
						.clone()
						.multiply(isect.material.ks.clone())
						.multiplyScalar(rdotvMax)
				); //specular formula
			color.add(idotkd.add(idotks)); //total formula
		}
	}
	// ---YOUR CODE ENDS HERE---
	return color;
